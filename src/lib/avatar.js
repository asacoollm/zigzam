...
// On exclut aussi `pouperEquipped` : la poupée équipée dépend d'un record
// détenu en temps réel (géré côté serveur), pas d'un accessoire du catalogue
// — une tenue sauvegardée ne doit ni la figer ni la retirer.
const LOADOUT_KEYS = Object.keys(DEFAULT_AVATAR)
  .filter((k) => k !== 'owned' && k !== 'pouperEquipped')

// Construit un avatar équipable depuis une tenue sauvegardée : on repart de
// l'avatar courant (donc de son `owned` ET de sa poupée équipée), et pour
// chaque catégorie on ne reprend la valeur de la tenue QUE si l'élève la
// possède encore (gratuite, ou présente dans `owned`). Les skins sur mesure
// retirés, les skins de saison non acquis, etc. sont donc silencieusement
// ignorés — ré-équiper une tenue n'ajoute jamais rien à `owned`.
